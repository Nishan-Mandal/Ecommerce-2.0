 ...
 * Centralized, category-generic Tax Invoice layout.
 * Shared between Admin print/preview and Client PDF rendering.
 */
  ...
  return (
    ...
    >
      ...
      <div className="border-t border-slate-200 pt-6 flex flex-col sm:flex-row justify-between items-center text-[11px] text-slate-500 gap-2">
        <p>Thank you for shopping with <strong className="text-slate-700">{company.name}</strong>!</p>
        <p className="text-center sm:text-right">Support: <span className="font-semibold text-slate-700">{company.email || company.phone || "Customer Desk"}</span></p>
      </div>
    </div>
  );
});
